...
const router = (app) => {
  ...
  app.post('/makePremium', mid.requiresLogin, mid.requiresSecure, controllers.Account.makePremium);
  // much like login this is where we are gonna set our currently selected board in the session
  app.post('/setCurrentBoard', mid.requiresLogin, mid.requiresSecure, controllers.Board.setCurrentBoard);
  app.get('/taskBoard', mid.requiresLogin, controllers.Taskboard.taskPage);
  app.post('/taskBoard', mid.requiresLogin, mid.requiresSecure, controllers.Taskboard.makeTask);
  // loads board page
  app.get('/boards', mid.requiresLogin, controllers.Board.boardPage);
  // adds a board to the users account
  app.post('/boards', mid.requiresLogin, mid.requiresSecure, controllers.Board.makeBoard);
  /// /////
